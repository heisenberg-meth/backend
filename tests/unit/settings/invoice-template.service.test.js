...
jest.unstable_mockModule(
  '../../../src/modules/settings/service/settings.prisma.service.js',
  () => ({ default: mockSettingsPrismaService }),
);
...
const { default: invoiceTemplateService } =
  await import('../../../src/modules/settings/invoice-template/invoice-template.service.js');
...
describe('InvoiceTemplateService', () => {
  ...
  describe('getTemplate', () => {
    it('should return template from settings service', async () => {
      mockSettingsPrismaService.getSettingsWithCache.mockResolvedValue({
      ...
      expect(mockSettingsPrismaService.getSettingsWithCache).toHaveBeenCalledWith(
        tenantId,
        'invoiceTemplate',
      );
    });
  ...
  describe('updateTemplate', () => {
    it('should update settings and create a version record', async () => {
      ...
      expect(mockSettingsPrismaService.updateCategorySettings).toHaveBeenCalledWith(
        tenantId,
        'invoiceTemplate',
        data,
        userId,
      ...
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('settings:invoice_template:updated', {
        tenantId,
        data,
      });
  ...
  describe('restoreVersion', () => {
    it('should restore config from a version ID', async () => {
      ...
      mockPrisma.invoiceTemplateVersion.findFirst.mockResolvedValue({
        id: versionId,
        versionNumber: 1,
        templateConfig,
      });
      ...
      expect(mockPrisma.invoiceTemplateVersion.findFirst).toHaveBeenCalledWith({
    ...
    it('should throw error if version not found', async () => {
      ...
      await expect(invoiceTemplateService.restoreVersion(tenantId, 'none', userId)).rejects.toThrow(
        'Invoice template version not found',
      );
    });
  });
  ...
  describe('testRender', () => {
    ...
    it('should warn about thermal overflow', async () => {
      const config = {
        ...
        footerText:
          'A very long footer text that exceeds forty two characters for testing overflow',
      };
      const result = await invoiceTemplateService.testRender(tenantId, config);
    ...
    it('should validate QR configuration', async () => {
      ...
      const result = await invoiceTemplateService.testRender(tenantId, config);
      expect(
        result.data.warnings.some((w) => w.includes('QR code enabled but GSTIN is missing')),
      ).toBe(true);
    });
  });
});
