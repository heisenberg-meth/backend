...
jest.unstable_mockModule(
  '../../src/modules/prescriptions/repositories/prescription.repository.js',
  () => ({
...
const { default: prescriptionService } =
  await import('../../src/modules/prescriptions/services/prescription.service.js');
const { default: verificationService } =
  await import('../../src/modules/prescriptions/services/verification.service.js');
...
describe('Clinical Workflow Unit Tests', () => {
  ...
  describe('PrescriptionService.validatePrescription', () => {
    ...
    it('should throw error if status is PENDING', async () => {
      ...
      await expect(
        prescriptionService.validatePrescription(prescriptionId, tenantId),
    ...
    it('should throw error and auto-expire if more than 6 months old', async () => {
      ...
      expect(mockPrescriptionRepository.updateStatus).toHaveBeenCalledWith(
        prescriptionId,
        tenantId,
        'EXPIRED',
      );
