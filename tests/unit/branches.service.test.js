...
const { default: transferService } =
  await import('../../src/modules/branches/services/transfer.service.js');
const { default: centralizedInventoryService } =
  await import('../../src/modules/branches/services/centralized-inventory.service.js');
...
describe('Multi-Branch Architecture Unit Tests', () => {
  ...
  describe('TransferService.requestTransfer', () => {
    ...
    it('should throw an error if source stock is insufficient', async () => {
      ...
      await expect(transferService.requestTransfer(tenantId, data, userId)).rejects.toThrow(
        'Insufficient stock in source branch for batch batch-1',
      );
    });
  });
