...
jest.unstable_mockModule(
  '../../src/modules/patients/repositories/prescription.repository.js',
  () => ({
...
const { default: loyaltyService } =
  await import('../../src/modules/patients/services/loyalty.service.js');
const { default: prescriptionService } =
  await import('../../src/modules/patients/services/prescription.service.js');
const { default: retentionService } =
  await import('../../src/modules/patients/services/retention.service.js');
const { default: recommendationService } =
  await import('../../src/modules/patients/services/recommendation.service.js');
...
describe('CRM Module Unit Tests', () => {
  ...
  describe('LoyaltyService.earnPoints', () => {
    it('should earn 1 point per ₹100 spent', async () => {
      ...
      expect(mockLoyaltyRepository.createTransaction).toHaveBeenCalledWith(
  ...
  describe('LoyaltyService.redeemPoints', () => {
    ...
    it('should throw error if insufficient points', async () => {
      ...
      await expect(
        loyaltyService.redeemPoints(tenantId, patientId, 100, mockPrisma),
  ...
  describe('LoyaltyService.expireOldPoints', () => {
    it('should expire points earned more than 1 year ago', async () => {
      ...
      mockPrisma.patient.findUnique = jest
        .fn()
        .mockResolvedValue({ id: patientId, loyaltyPoints: 150, tenantId });
  ...
  describe('RetentionService', () => {
    it('should identify chronic patients', async () => {
      mockPrisma.patient.findMany = jest
        .fn()
        .mockResolvedValue([{ id: patientId, fullName: 'Chronic Pat' }]);
