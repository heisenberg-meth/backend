...
async function teamRoutes(fastify) {
  ...
  fastify.get('/shifts', {
    ...
    schema: {
      ...
      querystring: {
        ...
        properties: {
          ...
          role: { type: 'string' },
          status: { type: 'string', enum: ['SCHEDULED', 'ACTIVE', 'COMPLETED', 'MISSED', 'OPEN', 'CLOSED'] },
    ...
    const shifts = await teamService.getShifts(request.tenantId, {
      userId, branchId, role, status, fromDate, toDate
    });
  ...
  fastify.post('/shifts', {
    ...
    const shift = await teamService.createShift(
      request.tenantId,
      request.body,
      request.user.id
    );
  ...
  fastify.get('/performance/overview', {
    preHandler: requireAdmin,
    schema: {
      tags: ['Team'],
  ...
  fastify.get('/billing-performance', {
    preHandler: requireAdmin,
    schema: {
      tags: ['Team'],
