...
const webhookWorker = isTest ? null : new Worker(
  'webhook-queue',
  async (job) => {
    ...
    if (provider === 'razorpay') {
      ...
      const isValid = razorpayWebhookHandler.verifySignature(body, signature || job.data.signature);
      if (!isValid) {
...
if (webhookWorker) {
  webhookWorker.on('failed', (job, err) => {
    logger.error({ job: job?.id, error: err.message, attempts: job?.attemptsMade }, '[WEBHOOK_WORKER] Job failed');
  });
}
