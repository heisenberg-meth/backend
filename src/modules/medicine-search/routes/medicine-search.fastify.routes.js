...
async function medicineSearchFastifyRoutes(fastify) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      let medicine = await medicineSearchRepository.findByBarcode(
        request.params.barcode,
        request.tenantId,
      );
      if (!medicine) {
        medicine = await medicineSearchRepository.findByBarcodeMapping(
          request.params.barcode,
          request.tenantId,
        );
      }
  ...
  fastify.get(
    ...
    async (request, reply) => {
      const medicine = await medicineSearchRepository.findBySku(
        request.params.sku,
        request.tenantId,
      );
      if (!medicine) return reply.code(404).send({ success: false, message: 'Medicine not found' });
