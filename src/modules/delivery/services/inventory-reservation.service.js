...
class InventoryReservationService {
  ...
  async reserveInventory(tenantId, orderItems) {
    logger.info(
      `[InventoryReservation] Reserving stock for order items: ${JSON.stringify(orderItems)}`,
    );
  ...
  async releaseReservation(tenantId, reservedBatches) {
    logger.info(
      `[InventoryReservation] Releasing reservations for batches: ${JSON.stringify(reservedBatches)}`,
    );
  ...
  async commitReservation(tenantId, reservedBatches) {
    logger.info(
      `[InventoryReservation] Committing reservations: ${JSON.stringify(reservedBatches)}`,
    );
  ...
  async returnToStock(tenantId, reservedBatches) {
    logger.info(
      `[InventoryReservation] Returning items to stock: ${JSON.stringify(reservedBatches)}`,
    );
