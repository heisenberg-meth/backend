...
async function alertFastifyRoutes(fastify) {
  ...
  fastify.post('/:alertId/snooze', { schema: { tags: ['Alerts'], summary: 'Snooze an alert' } }, alertController.snooze);
  fastify.post('/:alertId/mark-on-order', { schema: { tags: ['Alerts'], summary: 'Mark alert as on order' } }, alertController.markOnOrder);
  fastify.post('/:alertId/raise-po', { schema: { tags: ['Alerts'], summary: 'Raise PO from alert' } }, alertController.raisePurchaseOrder);
  fastify.post('/:alertId/resolve', { schema: { tags: ['Alerts'], summary: 'Resolve an alert' } }, alertController.resolve);
  fastify.post('/escalation-scan', { schema: { tags: ['Alerts'], summary: 'Trigger escalation scan' } }, alertController.triggerEscalationScan);
}
