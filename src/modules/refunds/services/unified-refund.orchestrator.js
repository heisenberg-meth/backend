...
class UnifiedRefundOrchestrator {
   ...
   */
  async processRefund({ tenantId, userId, invoiceId, items = [], refundAmount, reason, returnId = null, refundMethod = 'CASH' }) {
    return await prisma.$transaction(async (tx) => {
      ...
      if (targetInvoice.status === 'CANCELLED') throw new Error('Cannot refund a cancelled invoice');
      if (targetInvoice.status === 'REFUNDED') throw new Error('Invoice has already been fully refunded');
      ...
      if (items.length > 0) {
        ...
        for (const item of items) {
          const invItem = invoiceItems.find(i => i.id === item.invoiceItemId || i.medicineId === item.medicineId);
          if (!invItem) throw new Error(`Invoice item not found for refund`);
          if (item.quantity > invItem.quantity) {
             throw new Error(`Refund quantity (${item.quantity}) exceeds sold quantity (${invItem.quantity})`);
          }
      ...
      if (proposedTotal > invoiceTotal) {
        throw new Error(`Fraud Protection: Requested refund (₹${actualRefundAmount}) plus already refunded (₹${previouslyRefunded}) exceeds Invoice total (₹${invoiceTotal}).`);
      }
