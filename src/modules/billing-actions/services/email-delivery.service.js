...
class EmailDeliveryService {
  ...
  async sendInvoiceEmail(recipient, invoiceData, pdfBuffer, options = {}) {
    const { tenantName, branding } = options;

    logger.info(`[Email] Sending invoice to ${recipient}`);

    if (!process.env.EMAIL_USER) {
      logger.info(
        `[EMAIL_MOCK] Invoice email to: ${recipient} | Invoice: ${invoiceData.invoiceNumber}`,
      );
      return { success: true, messageId: `mock-email-${Date.now()}` };
    }

    const html = this.renderInvoiceEmail(invoiceData, tenantName, branding);
    ...
    const result = await this.transporter.sendMail({
      from: `"${tenantName || 'Viyan MedAssist'}" <${process.env.EMAIL_USER}>`,
      to: recipient,
      subject: `Invoice ${invoiceData.invoiceNumber} from ${tenantName || 'Viyan MedAssist'}`,
      html,
      attachments,
    });

    logger.info(`[Email] Invoice email sent to ${recipient}, messageId: ${result.messageId}`);

    return { success: true, messageId: result.messageId };
  }

  renderInvoiceEmail(invoiceData, tenantName, branding) {
    ...
    return `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="info-grid">
              ...
              <div class="info-item">
                ...
                <div class="info-value">${invoiceData.paymentStatus}</div>
              </div>
              <div class="info-item">
                <div class="info-label">GSTIN</div>
                <div class="info-value">${invoiceData.tenant?.gstin || 'N/A'}</div>
              </div>
            </div>
          ...
          <div class="footer">
            <p>This is a computer-generated invoice. Please find the PDF invoice attached.</p>
            <p>Powered by Viyan MedAssist</p>
          </div>
        </div>
      </body>
      </html>
