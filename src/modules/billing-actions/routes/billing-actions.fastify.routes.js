...
async function billingActionsFastifyRoutes(fastify) {
  ...
  fastify.post('/invoices/:id/print', {
    ...
    const printJob = await prisma.invoicePrintJob.create({
      data: { tenantId: request.tenantId, invoiceId: request.params.id, printerType: request.body.printerType, copies: request.body.copies, printStatus: 'PENDING' },
    });

    return reply.code(202).send({ success: true, data: { printJobId: printJob.id, status: printJob.printStatus } });
  });
  ...
  fastify.post('/invoices/:id/pdf', {
    ...
    const options = { watermark: request.body.watermark, duplicateCopy: request.body.duplicateCopy };
    ...
    await prisma.invoiceDeliveryLog.create({
      data: { tenantId: request.tenantId, invoiceId: request.params.id, deliveryChannel: 'PDF', recipient: 'system', deliveryStatus: 'DELIVERED', pdfUrl: signedUrl, expiresAt: new Date(Date.now() + 3600000) },
    });
  ...
  fastify.post('/invoices/:id/whatsapp', {
    ...
    await prisma.invoiceDeliveryLog.create({
      data: { tenantId: request.tenantId, invoiceId: request.params.id, deliveryChannel: 'WHATSAPP', recipient: request.body.phoneNumber, deliveryStatus: 'QUEUED' },
    });

    await invoiceDeliveryQueue.add({ invoiceId: request.params.id, channel: 'WHATSAPP', logId: 'log-1' });

    return reply.code(202).send({ success: true, data: { status: 'QUEUED', recipient: request.body.phoneNumber } });
  });
  ...
  fastify.post('/invoices/:id/email', {
    ...
    await prisma.invoiceDeliveryLog.create({
      data: { tenantId: request.tenantId, invoiceId: request.params.id, deliveryChannel: 'EMAIL', recipient: request.body.email, deliveryStatus: 'QUEUED' },
    });

    await invoiceDeliveryQueue.add({ invoiceId: request.params.id, channel: 'EMAIL', logId: 'log-1' });

    return reply.code(202).send({ success: true, data: { status: 'QUEUED', recipient: request.body.email } });
  });
  ...
  fastify.get('/invoices/:id/download', {
    ...
    const result = await pdfGenerationService.generateAndStore(request.params.id, request.tenantId);
    return reply.send({ success: true, data: { pdfUrl: result.pdfUrl } });
  });

  fastify.get('/invoices/:id/delivery-status', {
  ...
  fastify.post('/invoices/:id/resend', {
    ...
    for (const channel of request.body.channels) {
      const log = await prisma.invoiceDeliveryLog.create({
        data: { tenantId: request.tenantId, invoiceId: request.params.id, deliveryChannel: channel.toUpperCase(), recipient: channel === 'email' ? request.body.email : request.body.phoneNumber, deliveryStatus: 'QUEUED' },
      });
    ...
    for (const invoiceId of request.body.invoiceIds) {
      ...
      if (invoice) {
        await prisma.invoicePrintJob.create({
          data: { tenantId: request.tenantId, invoiceId, printerType: request.body.printerType, copies: request.body.copies, printStatus: 'PENDING' },
        });
        totalQueued++;
      }
    }
  ...
  fastify.post('/invoices/:id/regenerate-pdf', {
    ...
    const result = await pdfGenerationService.generateAndStore(request.params.id, request.tenantId, {
      watermark: request.body.reason === 'cancelled' ? 'CANCELLED INVOICE' : request.body.reason,
  ...
  fastify.get('/invoices/:id/print-history', {
