...
export const registerSchema = z.object({
  email: z.string({ required_error: 'Email is required' }).email('Valid email address required').toLowerCase().trim(),
...
export const verifyResetOtpSchema = z.object({
  email: z.string({ required_error: 'Email is required' }).email('Valid email address required').toLowerCase().trim(),
  otp: z.string({ required_error: 'OTP is required' }).length(6, 'OTP must be exactly 6 digits').regex(/^\d{6}$/, 'OTP must be numeric'),
});
